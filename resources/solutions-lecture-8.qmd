...
The maximum likelihood estimator is $\overline{x}$. The maximum likelihood estimator of $\sin(\lambda)$ is $\sin(\overline{x})$ by the invariance principle.
...
samples = rng.exponential(size = (10000, 100), scale = 2).mean(axis = 1)
...
```{python}
def sample(n):
  theta_hats = rng.exponential(size = (10000, n), scale = 2).mean(axis = 1)
...
$\log (\lambda\exp[-\lambda x]) = \log(\lambda) - \lambda x$, so its derivative is $1/\lambda-x$ and second derivative $-1/\lambda$.
...
The indefinite integral is $\int_ 1/b dx = x/b$. Now plug in the limits.

> 2.  Calculate the expectation of $X \sim f(x, b)$. Can you imagine a reasonable estimator of $b$?

The expectation is $b/2$ as $\int x dx = x^2/2$. When you plug in the limits. A reasonable estimator for $b$ is $2*E(X)$.

> 3.  Try to show that the maximum likelihood estimator of $b$ is $\max_{i=1}^n x_i$. (*Hint:* Don't use differentiation!)

The smaller the upper limit $b$ is, the higher the likelihood, as long as every observation is smaller than or equal to $b$. (If $x>b$ the likelihood is $0$!) It follows that $\hat{b} = \max_{i=1}^n x_i$.
