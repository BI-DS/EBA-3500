...
```
#### Quick exercise
Assume the variance of the `calories` data is known and equal to $\sigma^2=0.2$. Compute the $95\%$-confidence interval (i.e., $\alpha = 0.05$) of the form $(L_n, U_n)=(\overline{X}_n - z_{\alpha/2}\frac{\sigma}{\sqrt{n}}, \overline{X}_n + z_{\alpha/2}\frac{\sigma}{\sqrt{n}})$.

::: {.callout-tip collapse="true"}
#### Solution
Verify that the sample mean of the `calories` data is approximately $23.788$ and $n=23$.  
Use the approximation $z_{0.05/2}\approx 1.96$ and plug in $\sigma = \sqrt{24}$, $\overline{X}_n = 23.788$ into the formula for the confidence interval to obtain:
...
$$(L_n, U_n)=(\overline{X}_n - \frac{t_{n-1,\alpha/2}}{\sigma\sqrt{n}}, 
              \overline{X}_n + \frac{t_{n-1,\alpha/2}}{\sigma\sqrt{n}}).$$
              
Such confidence intervals can be calculated using Scipy.
```{python}
...
interval for the mean equal to [0.4, 0.6]. Which, if any, of the following are true?
