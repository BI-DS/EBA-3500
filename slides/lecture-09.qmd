...
Let $\hat{\theta}$ be a maximum likelihood estimator and $\theta$ be the true parameter value. Then $\hat{\theta}$ is asymptoticall normal with limiting variance equal to the inverse Fisher information, i.e., $$\sqrt{n}(\hat{\theta}-\theta) \to N(0, I(\theta)^{-1})$$
:::
:::
...
-   But often you have to make a make a prediction of $\hat{Y}$ itself, e.g., will you follow a marketing lead? Do you believe candidate will get an $A$ when he drinks 5 cups of coffee?
...
-   **Question:** How many true positives are there?
    -   **Answer:** True positives happen when both $\hat{Y}=1$ and $Y=1$. Hence the correct answer is $254$.
-   **Question:** How many false negatives are there?
    -   **Answer:** False negatives correspond to $\hat{Y}=0$ and $Y=1$. There are 19 of these.

## Confusion matrix for `admission` with `c = 0.6`
...
6.  To predict $Y$ from fitted probabilities you need to choose a threshold. Each threshold is associated with a confusion matrix, displaying the relationsship between $\hat{Y}$ and $Y$.
7.  The ROC displays the relationsship between the true positive rate and the false negative rate.
8.  The AUC measures how good your model is at predicting, with $0.5$ being no better than chance and $1$ being perfect. The AUC should not be used to compare models. Use the AIC for that.
9.  McFadden's $R^2$ is a generalization of the $R^2$ to binary models.
